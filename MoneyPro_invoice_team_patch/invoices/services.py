...
def mark_paid(*, invoice: Invoice, paid_date=None) -> Transaction:
    ...
    posting_subcategory = first.subcategory
    if posting_subcategory.requires_team and not invoice.team_id:
        raise ValueError(
            f"The posting subcategory '{posting_subcategory.name}' requires a team. "
            "Edit the invoice and select a team, then try Mark Paid again."
        )
    ...
    t = Transaction.objects.create(
        ...
        subcategory=posting_subcategory,
        contact=invoice.contact,
        team=invoice.team,
        job=invoice.job,
        invoice_number=invoice.invoice_number,
